...
struct dbtype Namedb = {
      {{{0xE2,0x00,0x40,0x05,0x73,0x07,0x00,0x67,0x22,0x90,0x28,0xF7},"Ruben"},    //1b
      {{0xE2,0x00,0x00,0x15,0x86,0x0E,0x01,0x31,0x15,0x40,0x7E,0xF2},"Rowan"},    //2
      {{0xE2,0x00,0x20,0x00,0x20,0x00,0x00,0x00,0x15,0x64,0x46,0x3C},"Brendan"}}  //paperclip
};
      ...
      if(memcmp(lastid, id, sizeof(id)) != 0) {
        ...
        for(uint16_t i=0; i<dbsize; i++) {
          if(memcmp(id, (Namedb.Users[i].serialno), 12) == 0) {
            Serial.print(Namedb.Users[i].username);
            Serial.print("\r\n");
          }
        }
      }
